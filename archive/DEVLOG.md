...
Trigger to revisit the persistence question: corpus past roughly 30k chunks (where the rebuild scales toward 5 seconds and starts being noticeable on the first query after process start) or the MCP server seeing frequent restarts in deployment. Persisting via pickle plus an invalidation hash (chunk count + sorted chunk_ids) is roughly 50 lines, but it adds a stale-cache risk surface that the rebuild-every-start approach doesn't have. An alternative path is adopting Chroma's native hybrid search (newer versions support it), which would delete the hand-rolled BM25 code entirely but requires a Chroma version upgrade and might shift the eval numbers the v2 to v5 narrative depends on.
...
## BM25 Tokenizer Ablation (v6)

A code review on the post-v5 codebase pointed out that the BM25 tokenizer (`doc.lower().split()`) was the simplest possible choice and that I'd never measured whether a richer tokenizer would help. The note was fair: I'd considered tokenization but not run numbers on it.

I built a small ablation rig at `tests/ablation/bm25_tokenization.py`. It runs the full retrieval pipeline (hybrid + cross-encoder rerank) against the 30-question golden set under six tokenizer variants and reports Recall@k, MRR, NDCG@5 per variant, plus a per-question-type breakdown.

The variants:

- `baseline`: `text.lower().split()` (the v5 tokenizer).
- `porter`: baseline + Porter stemming. Collapses morphological variants ("embeddings" matches "embedding", "trained" matches "training").
...
Aggregate Recall@5: baseline 0.517, porter 0.586, nostop 0.517, hyphen 0.621, porter+nostop 0.552, **porter+hyphen 0.690**. MRR moves with it: 0.378 baseline to 0.543 at porter+hyphen.

`nostop` slightly hurt the rank-1 metric (0.310 to 0.276). BM25's IDF already de-weights common terms, so stripping them explicitly was removing signal. Documented as a negative result and not adopted.

`porter+hyphen` is the clear winner. The per-type breakdown showed no regressions vs baseline anywhere, and two breakthrough wins: `equation` went 0.500 to 1.000 (both equation questions now retrieved), and `table_numerical` finally moved off zero (0.000 to 0.333) for the first time in the project. `multi_hop` and `vague_ambiguous` stayed at zero, as predicted for agent-layer problems.

Why hyphen mattered specifically: the v4 contextual chunking work made sure every chunk contained the paper's distinctive name. The v5 tokenizer was undermining that by keeping `SARLO-80` as one token that queries naming `SARLO` couldn't match. Splitting on hyphens makes v4's contextual prefix actually pay off in BM25 ranking. The two changes compose.

Implementation: `mini_rag/retriever._default_tokenizer` is now `[porter.stemWord(t) for t in re.findall(r"\w+", text.lower())]`. Both corpus indexing and query tokenization use the same function. `snowballstemmer>=2.2.0` is a main runtime dep from v6 onward. Ablation results are persisted at `tests/ablation/bm25_tokenization_results.json` for reproducibility.

Lemmatisation was considered as a richer alternative (dictionary-based normalisation producing real words, vs stemming's rule-based truncation that produces non-words like "studi"). For BM25 specifically the index doesn't care if the tokens are real words, only that they're consistent, so stemming usually wins on cost-benefit. Filed as a future direction not built now.

After adoption I re-ran the main eval (tests/eval/run_eval.py) against the full 30-question set so the LLM-judged RAGAS layer had v6 numbers too. The retrieval side matches the ablation exactly (Recall@5 = 0.690, MRR = 0.541), which confirms the production default is what I measured. RAGAS deltas from v5: faithfulness 0.826 to 0.882 (+0.056), context recall 0.608 to 0.619 (+0.011), context precision 0.746, answer relevancy 0.651. Answer correctness dipped slightly: 0.474 to 0.458 (-0.016).

I'm flagging the answer-correctness dip rather than hiding it. It's probably within LLM-judge variance at n=30 (this is the metric that compares the generated answer against the gold answer via an LLM similarity call, and small wording differences can move the score). A plausible mechanism if it's real: richer retrieval pulls in more relevant context, and the minimal generator occasionally picks up adjacent phrasing that shifts the wording of the answer even when the substance is right. Retrieval is unambiguously better; the answer-generation surface is where any noise now lives.
